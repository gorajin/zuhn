---
id: INS-260321-6D01
domain: music-production
topic: songwriting
title: Best songs come from solo marination followed by collaborative refinement
actionability: immediate
confidence: high
shelf_life: evergreen
status: active
tags:
  - creative-process
  - solo-writing
  - collaboration
  - songwriting-workflow
sources:
  - type: youtube
    title: 'Ep. 243: Charlie Puth | The Long Road to Becoming Yourself'
    author: And The Writer Is...
    url: 'https://youtu.be/bxovEgjRwZg'
date_extracted: '2026-03-21'
last_accessed: null
access_count: 0
indexed: true
embedded: true
embedding_model: nomic-embed-text
resolutions:
  one_line: >-
    Puth's best work follows a two-phase pattern: solo marination where the song
    develops in his head, then collaboration where others refine and constrain
    his tendencies.
  standard: >-
    Charlie Puth identifies his optimal creative workflow: Phase 1 is writing
    alone, letting the song marinate in his head — something impossible with
    collaborators present because the pressure to perform takes over. Phase 2 is
    bringing in collaborators (like Cash, Joe, or Blood Pop) to 'check' him —
    preventing the music from getting 'too jazzed' for pop. The problem with his
    current fame: when he enters writing sessions, the focus shifts to him
    rather than the artist he's writing for. Everyone looks to him for answers,
    which kills the dynamic. His solution: become more selective about sessions
    and prioritize artists who maintain their own creative gravity rather than
    deferring to his reputation.
related:
  - INS-260321-E9B7
  - INS-260321-9C2E
  - INS-260321-3312
  - INS-260321-33FC
  - INS-260321-F0DF
---
Charlie Puth identifies his optimal creative workflow: Phase 1 is writing alone, letting the song marinate in his head — something impossible with collaborators present because the pressure to perform takes over. Phase 2 is bringing in collaborators (like Cash, Joe, or Blood Pop) to 'check' him — preventing the music from getting 'too jazzed' for pop. The problem with his current fame: when he enters writing sessions, the focus shifts to him rather than the artist he's writing for. Everyone looks to him for answers, which kills the dynamic. His solution: become more selective about sessions and prioritize artists who maintain their own creative gravity rather than deferring to his reputation.
